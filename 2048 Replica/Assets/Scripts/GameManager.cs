...
public class GameManager : MonoBehaviour {
	...
	//private List <Tile[]> UndoTiles = new List<Tile[]>();
	//private Tile[] Undoarray = new Tile[16];
	private int [] UndoValues= new int[16];
...
//			} 
//			else if (State == GameState.NotPlaying) 
//			{
...
//			{
	...
	void addelements()
	{
		for(int i=0; i<4; i++)
		{
			...
			{
				int k = i*4+j;
				UndoValues [k] = AllTiles [i, j].Number;
			}
		}
	...
	public void Undomove()
	{
		if (State == GameState.GameOver) 
		{
			...
			foreach (Tile t in AllTiles) 
			{
				int i = t.indRow * 4 + t.indCol;
				AllTiles [t.indRow, t.indCol].Number = UndoValues [i];
			}
			State = GameState.Playing;
		} 
		else 
